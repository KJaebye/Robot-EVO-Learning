...
class AugmentMujocoXmlEnv(gym.Env):
    """
    ...
    def _reset(self):
        ...
        xml_filename = os.path.join(os.path.dirname(__file__), "mujoco_assets", self.model_xml)
        orig_xml_filename = os.path.join(os.path.dirname(__file__), "mujoco_assets", self.orig_model_xml)
        if os.path.isfile(xml_filename) and os.path.getsize(xml_filename) > 500:
          self.mjcf = self.scene.cpp_world.load_mjcf(xml_filename)
        else:
          print("Error Loading", self.model_xml)
          self.mjcf = self.scene.cpp_world.load_mjcf(orig_xml_filename)
        self.ordered_joints = []
        self.jdict = {}
        self.parts = {}
        self.frame = 0
        ...
        self.reward = 0
        dump = 0
        if dump: print("reloading urdf from", xml_filename)
        for r in self.mjcf:
